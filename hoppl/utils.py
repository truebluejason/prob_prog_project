...
def plot_sample_trace(plotname, traces):
    ...
    if dimension == 1:
        plt.plot(traces)
        plt.savefig(f'figures/hw5/{plotname}_0.png')
        plt.clf()
    else:
        ...
        plt.clf()
        plt.plot(d2)
        plt.savefig(f'figures/hw5/{plotname}_1.png')
        plt.clf()

def plot_losses(plotname, losses):
    plt.plot(losses)
    plt.savefig(f'figures/hw5/{plotname}.png')
    plt.clf()
...
def plot_posterior(plotname, traces, weights=None):
    ...
    for i in range(num_latent):
        ...
        plt.savefig(f'figures/hw5/{plotname}_{i}.png')
        plt.clf()
...
def plot_posterior_jointly(plotname, traces, x_dim, y_dim, weights=None):
    ...
    plt.tight_layout()
    plt.savefig(f'figures/hw5/{plotname}_joint.png')
