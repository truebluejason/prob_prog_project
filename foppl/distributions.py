...
#         if high > low:
#             self.optim_high = high.clone().detach().requires_grad_()
#         else:
#             self.optim_high = torch.log(torch.exp(high) - 1).clone().detach().requires_grad_()
#         super().__init__(low, torch.nn.functional.softplus(self.optim_high))

#     def Parameters(self):
#         """Return a list of parameters for the distribution"""
#         return [self.low, self.optim_high]
...
#     def log_prob(self, x):
#         self.high = torch.nn.functional.softplus(self.optim_high)
#         return super().log_prob(x)
